...
SYMBOL = "XAU/USD"
CHECK_INTERVAL = 300  # secondi tra ogni controllo (5 minuti, per restare sotto 800 API calls/giorno)
...
ACCOUNT_BALANCE = 448.0   # capitale del conto in EUR
RISK_PERCENT    = 1.0     # % di rischio per trade (1% = conservativo)
...
def calc_position_size(price: float, sl: float) -> tuple:
    """
    Calcola la size consigliata (in lotti) in base al rischio % del capitale.
    Su XAU/USD: 1 lotto standard = 100 once, 1 punto di movimento = $100 P&L.
    0.01 lotti (micro) = $1 per punto di movimento.
    """
    risk_amount = ACCOUNT_BALANCE * (RISK_PERCENT / 100)  # EUR a rischio
    sl_distance = abs(price - sl)  # distanza in punti/dollari

    if sl_distance == 0:
        return 0.01, risk_amount

    # 1 lotto standard = $100/punto, quindi lotti = risk_amount / (sl_distance * 100)
...
def format_message(direction: str, score: float, price: float, sl: float, tp: float,
                   atr: float, rsi: float, details: list, lots: float, risk_eur: float) -> str:
    ...
    return (
        ...
        f"🛑 *Stop Loss:* `{sl}`\n"
        f"✅ *Take Profit:* `{tp}`\n"
        f"📦 *Size consigliata:* `{lots}` lotti (rischio {RISK_PERCENT}% = {risk_eur}€)\n"
        f"📊 *ATR:* `{atr}` | *RSI:* `{rsi}`\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"*Dettaglio segnali:*\n{detail_text}"
    )
...
# Flag per evitare doppio alert apertura mercato
market_open_alert_sent_date = None

# Cache per i timeframe lenti (M15, H1, H4) - si aggiornano ogni 15 minuti
slow_tf_cache = {
    "last_update": 0,
    "df_m15": None,
    "df_h1": None,
...
SLOW_TF_REFRESH = 900  # 15 minuti in secondi
...
def check_trailing_stop(price: float):
    ...
    if direction == "BUY" and price >= tp:
        ...
        send_telegram("🎯 *TARGET RAGGIUNTO! BUY XAU/USD*\nEntry: `" + str(round(entry,2)) + "` TP: `" + str(round(tp,2)) + "`\n+" + str(pnl) + " punti\n\n📊 Stats: " + str(stats["wins"]) + "W/" + str(stats["losses"]) + "L (" + str(winrate) + "%) | Tot: " + str(round(stats["pnl_total"],1)) + " pts")
    ...
    if direction == "BUY" and price <= active_trade["sl"]:
        pnl = round(active_trade["sl"] - entry, 2)
        stats["total"] += 1
        stats["losses"] += 1
        stats["pnl_total"] += pnl
        ...
        pnl = round(entry - active_trade["sl"], 2)
        stats["total"] += 1
        stats["losses"] += 1
        stats["pnl_total"] += pnl
        winrate = round(stats["wins"] / stats["total"] * 100, 1) if stats["total"] else 0
...
# Cache per le news del giorno (per non richiamare l'API troppo spesso)
news_cache = {"date": None, "events": []}

def get_high_impact_news():
    """
    Recupera eventi economici ad alto impatto per oggi da Forex Factory (JSON pubblico).
    Ritorna lista di datetime UTC degli eventi ad alto impatto su USD.
    """
    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if news_cache["date"] == today_str:
    ...
    try:
        ...
        for item in data:
            if item.get("country") == "USD" and item.get("impact") == "High":
                event_time_str = item.get("date")  # ISO format
                try:
                    ...
                    continue
        news_cache["date"] = today_str
        news_cache["events"] = events
        print(f"  News ad alto impatto oggi: {len(events)}")
    except Exception as e:
        print(f"  Errore fetch news: {e}")
    return events

def is_near_news_event() -> bool:
    """Blocca segnali 30 min prima/dopo eventi ad alto impatto USD."""
...
def check_market_open_alert():
    """Manda un alert 30 minuti prima dell'apertura sessione Londra (09:00 UTC)."""
    global market_open_alert_sent_date
    now = datetime.now(timezone.utc)
    today_str = now.strftime("%Y-%m-%d")
    if now.hour == 8 and 25 <= now.minute <= 35:
        if market_open_alert_sent_date != today_str:
            events = get_high_impact_news()
            news_text = ""
            if events:
                events_str = ", ".join([e.strftime("%H:%M UTC") for e in events])
                news_text = f"\n📰 News alto impatto oggi: {events_str}"
            send_telegram(f"⏰ *Mercato Londra apre tra 30 min!*\nXAU/USD sessione attiva alle 09:00 UTC{news_text}")
            market_open_alert_sent_date = today_str

def is_trading_session() -> bool:
    """Blocca segnali fuori dalle sessioni attive (07:00-22:00 UTC)."""
    hour = datetime.now(timezone.utc).hour  # ora UTC esatta
    return 7 <= hour < 22
...
def run():
    ...
    while True:
        try:
            ...
            # Controlla se è ora di mandare l'alert apertura mercato
            check_market_open_alert()

            df_m1  = fetch_candles("1min",  outputsize=50)
            time.sleep(1)
            df_m5  = fetch_candles("5min",  outputsize=100)

            # Timeframe lenti (M15, H1, H4): aggiorna solo ogni 15 minuti per risparmiare API
            now_cache_ts = time.time()
            if (now_cache_ts - slow_tf_cache["last_update"]) >= SLOW_TF_REFRESH or slow_tf_cache["df_m15"] is None:
                time.sleep(1)
                slow_tf_cache["df_m15"] = fetch_candles("15min", outputsize=100)
                time.sleep(1)
                slow_tf_cache["df_h1"]  = fetch_candles("1h",    outputsize=100)
                time.sleep(1)
                slow_tf_cache["df_h4"]  = fetch_candles("4h",    outputsize=100)
                slow_tf_cache["last_update"] = now_cache_ts
                print("  (Timeframe lenti M15/H1/H4 aggiornati)")

            df_m15 = slow_tf_cache["df_m15"]
            df_h1  = slow_tf_cache["df_h1"]
            df_h4  = slow_tf_cache["df_h4"]
            ...
            # Blocca segnali vicino a news ad alto impatto (±30 min)
            if direction and is_near_news_event():
                print(f"  Vicino a news ad alto impatto, segnale {direction} ignorato")
                direction = None
            ...
            if direction:
                ...
                if not (same_dir and too_recent):
                    ...
                    lots, risk_eur = calc_position_size(price, sl)
                    msg = format_message(direction, score, price, sl, tp, atr, rsi, details, lots, risk_eur)
                    send_telegram(msg)
